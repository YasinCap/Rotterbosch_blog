...
export default function Blogposts({ post }) {
  ...
  return (
    <div className="container px-10 mb-8 rounded ">
      <Navbar></Navbar>
      <div className=" grid-cols-1 lg:grid-cols-6 gap-8 bg-grey-50 py-4" key={post.sys.id}>
        <p className="text-3xl m-auto">
          {post.fields.title}
        </p>
        
        <img className="object-cover h-48 w-96 m-auto rounded py-1" src={post.fields.coverImage.fields.file.url}></img>
        
        <p>{post.fields.content.content[0].content[0].value}</p>
        <br></br>

        <h3 className="text-sm">{post.fields.author.fields.name}</h3>
        <img className="w-10 rounded" src={post.fields.author.fields.picture.fields.file.url}></img> <p className="text-sm">{post.fields.date}</p>
        <br></br>
      </div>
    </div>
  )
